...
use crate::{
    helix_engine::graph_core::{
        ops::source::{n_from_index::NFromIndexAdapter, n_from_type::NFromTypeAdapter},
        traversal_iter::collect_parallel,
    },
    protocol::{
...
#[test]
fn test_collect_parallel() {
    let (storage, _temp_dir) = setup_test_db();
    let txn: heed3::RoTxn<'_, heed3::WithoutTls> = storage.graph_env.read_txn().unwrap();
    ...
    // let traversal = traversal.map(|item| item.enumerate());

    let result = collect_parallel(traversal);
    println!("{:?}", result);
}
