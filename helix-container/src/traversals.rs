...
pub fn get_user(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let mut user = TraversalBuilder::new(Arc::clone(&db), TraversalValue::Empty, Some(txn), None);
    ...
    let txn = db.env.read_txn().unwrap();
    let mut follower_edges =
        TraversalBuilder::new(Arc::clone(&db), user.current_step.clone(), Some(txn), None);
    follower_edges.in_e("follows");
    let txn = db.env.read_txn().unwrap();
    let mut followers = TraversalBuilder::new(
        Arc::clone(&db),
        follower_edges.current_step.clone(),
        Some(txn),
        None,
    );
    followers.out_v();
...
pub fn get_user_subgraph(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let txn = db.env.read_txn().unwrap();
    let mut mutuals = TraversalBuilder::new(Arc::clone(&db), user.clone(), Some(txn), None);
    mutuals.mutual("follows").range(0, 300);
    let mutuals = mutuals.result().unwrap();
    println!("Mutuals traversal took: {}ms", start_mutuals.elapsed().as_millis());
    
    // get 200 random nodes
    let start_random = std::time::Instant::now();
    let txn = db.env.read_txn().unwrap();
    let mut random = TraversalBuilder::new(Arc::clone(&db), TraversalValue::Empty, Some(txn), None);
    random.v().range(0, 200);
    let random = random.result().unwrap();
    println!("Random nodes traversal took: {}ms", start_random.elapsed().as_millis());
    ...
    return_vals.insert("random".to_string(), ReturnValue::TraversalValues(random));
    println!("Insertion took: {}ms", insert_time.elapsed().as_millis());
    ...
    println!("JSON serialization took: {}ms", start_serialize.elapsed().as_millis());
    Ok(())
}
