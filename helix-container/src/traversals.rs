...
pub fn get_shortest_path_to_user(
    ...
    let txn = db.graph_env.read_txn().unwrap();
    let mut to_user = TraversalBuilder::new(Arc::clone(&db), TraversalValue::Empty);
    to_user.v_from_secondary_index(&txn, "username", &Value::String(data.to_username));
    let to_user_id = to_user.finish()?.get_id()?;

    let mut user = TraversalBuilder::new(Arc::clone(&db), TraversalValue::Empty);
    user.v_from_secondary_index(&txn, "username", &Value::String(data.username))
        .shortest_mutual_path_to(&txn, to_user_id.as_str());
...
pub fn update_user(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    match tr.current_step {
        ...
        _ => {
            return Err(GraphError::from("User not found".to_string()));
        }
    }
...
pub fn upload_all(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    let d: Data<'_> = sonic_rs::from_slice::<Data>(&input.request.body).unwrap();
    let mut return_vals: HashMap<String, ReturnValue> = HashMap::with_capacity(1);
    let db = Arc::clone(&input.graph.storage);
