...
pub fn get_all_venues(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    tr.current_step = TraversalValue::from(venues.clone());
...
pub fn update_venue(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        let id = tr.finish()?;
        let id_remapping = Remapping::new(false, Some("id".to_string()), Some());
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        let venue_info = tr.finish()?;
        let venue_info_remapping = Remapping::new(
            false,
...
pub fn get_user_saved_events(
    ...
    tr.current_step = TraversalValue::from(events.clone());
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        tr.out(&txn, "Booking");
        tr.for_each_node(&txn, |item, txn| {
        ...
        let venue_remapping = Remapping::new(false, Some("venue".to_string()), Some());
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        tr.for_each_node(&txn, |item, txn| {
        ...
        tr.for_each_node(&txn, |item, txn| {
            ...
            let event_name_remapping = Remapping::new(false, Some("eventName".to_string()), Some());
            let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
            tr.for_each_node(&txn, |item, txn| {
            ...
            let start_date_time_remapping =
                Remapping::new(false, Some("startDateTime".to_string()), Some());
            let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
            tr.for_each_node(&txn, |item, txn| {
            ...
            let end_date_time_remapping =
                Remapping::new(false, Some("endDateTime".to_string()), Some());
            remapping_vals.borrow_mut().insert(
                item.id.clone(),
                ResponseRemapping::new(
                    HashMap::from([
        ...
        let bookings_for_event_remapping =
            Remapping::new(false, Some("bookingsForEvent".to_string()), Some());
        remapping_vals.borrow_mut().insert(
            item.id.clone(),
            ResponseRemapping::new(
                HashMap::from([
...
pub fn get_bookings_for_event(
    ...
    tr.current_step = TraversalValue::from(bookings.clone());
    tr.in_v(&txn);
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        let event_id = tr.finish()?;
        let event_id_remapping = Remapping::new(false, Some("eventID".to_string()), Some());
        let event_name = item.check_property("eventName");
        let event_name_remapping = Remapping::new(
            false,
            None,
...
pub fn create_event(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    tr.current_step = TraversalValue::from(event.clone());
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        let venue = tr.finish()?;
        let venue_remapping = Remapping::new(false, Some("venue".to_string()), Some());
...
pub fn get_all_venues_and_bookings(
    ...
    tr.current_step = TraversalValue::from(venuesData.clone());
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        let id = tr.finish()?;
    ...
    tr.current_step = TraversalValue::from(venueBookingsData.clone());
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        let id = tr.finish()?;
        let id_remapping = Remapping::new(
...
pub fn create_venue_booking(
    ...
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        tr.current_step = TraversalValue::from(venue_V.clone());
        tr.for_each_node(&txn, |item, txn| {
            let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
            let venue_info = tr.finish()?;
            let venue_info_remapping = Remapping::new(
...
pub fn get_user_event_bookings(
    ...
    tr.current_step = TraversalValue::from(eventBookings.clone());
    tr.for_each_node(&txn, |item, txn| {
        let mut tr = TraversalBuilder::new(Arc::clone(&db), TraversalValue::from(item.clone()));
        tr.for_each_node(&txn, |item, txn| {
            let id_remapping = Remapping::new(false, None, None);
