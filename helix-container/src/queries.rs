...
pub fn file9(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let path1 = G::new(Arc::clone(&db), &txn)
        .n_from_id(&data.id)
        .shortest_path(Some("File9"), None, Some(&data.other_id))
        .collect_to::<Vec<_>>();
    let path2 = G::new(Arc::clone(&db), &txn)
        .n_from_id(&data.id)
        .shortest_path(Some("File9"), Some(&data.other_id), None)
        .collect_to::<Vec<_>>();
    return_vals.insert(
        "path1".to_string(),
        ReturnValue::from_traversal_value_array_with_mixin(path1, remapping_vals.borrow_mut()),
    );

    return_vals.insert(
        "path2".to_string(),
        ReturnValue::from_traversal_value_array_with_mixin(path2, remapping_vals.borrow_mut()),
    );
