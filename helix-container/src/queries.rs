...
pub fn ragloaddocs(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    for data in data.docs {
        ...
        for data in vectors { // TODO: needs to be vectors not data.vecs
            let tr = G::new_mut(Arc::clone(&db), &mut txn)
                .add_e("Contains", props!{}, None, doc_node.id(), data.id(), false, true); // TODO: from_is_vec, to_is_vec
...
pub fn ragtestload(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let tr = G::new_mut(Arc::clone(&db), &mut txn)
        .add_e("Contains", props!{}, None, doc_node.id(), vector.id(), false, true); // TODO: need to add from_is_vec, to_is_vec
