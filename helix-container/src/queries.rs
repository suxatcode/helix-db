...
pub fn ragsearchdocs(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let tr = G::new_from(Arc::clone(&db), &txn, vec.clone()).in_e("EmbeddingOf");
        ...
        .map(|item| {
            match item {
                Ok(ref item) => {
                    ...
                    let chunk_remapping = Remapping::new(
                        false,
                        None,
                        Some(match chunk {
                    ...
                    remapping_vals.borrow_mut().insert(
                        ...
                        ResponseRemapping::new(
                            HashMap::from([("chunk".to_string(), chunk_remapping)]),
                            false,
                        ),
                    );
                }
    ...
    for data in data.docs {
        ...
        for data in data.vectors {
            ...
            let tr = G::new_mut(Arc::clone(&db), &mut txn).add_e(
                "EmbeddingOf",
                props! { "chunk".to_string() => data.chunk },
                None,
                doc_node.id(),
                vec.id(),
                true,
