...
use helixdb::protocol::items::{v6_uuid, Edge, Node};
use helixdb::protocol::label_hash::hash_label;
use helixdb::{
...
pub fn bulk_loader(input: &HandlerInput, response: &mut Response) -> Result<(), GraphError> {
    ...
    let edges: Arc<Mutex<Vec<(u128, u128, u128)>>> =
        Arc::new(Mutex::new(Vec::with_capacity(1_600_000_000)));

    // Create thread handles
    let mut handles = Vec::with_capacity(num_threads);

    let line_count = Arc::new(AtomicU64::new(0));
    let hash_label = hash_label("knows", None);
    for i in 0..num_threads {
        let start_pos = i as u64 * chunk_size;
        let file_path = file_path.to_string();
        let nodes_clone: Arc<Mutex<HashSet<u128>>> = Arc::clone(&nodes);
        let edges_clone: Arc<Mutex<Vec<(u128, u128, u128)>>> = Arc::clone(&edges);
        ...
        let handle = thread::spawn(move || {
            ...
            for line in reader.lines() {
                ...
                local_edges.push((from_id, to_id, v6_uuid()));
